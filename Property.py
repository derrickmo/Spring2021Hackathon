...
class List:
    ...
    def __init__(self, input, lat, lon, weight):
        ...
        for i in range(len(self.listProperty)):
            self.priceScoreMin = self.listProperty[i].priceScore if self.listProperty[i].priceScore < self.priceScoreMin else self.priceScoreMin
            self.priceScoreMax = self.listProperty[i].priceScore if self.listProperty[i].priceScore > self.priceScoreMax else self.priceScoreMax
            self.distanceScoreMin = self.listProperty[i].distanceScore if self.listProperty[i].distanceScore < self.distanceScoreMin else self.distanceScoreMin
            self.distanceScoreMax = self.listProperty[i].distanceScore if self.listProperty[i].distanceScore > self.distanceScoreMax else self.distanceScoreMax
            self.usabilityScoreMin = self.listProperty[i].usabilityScore if self.listProperty[i].usabilityScore < self.usabilityScoreMin else self.usabilityScoreMin
            self.usabilityScoreMax = self.listProperty[i].usabilityScore if self.listProperty[i].usabilityScore > self.usabilityScoreMax else self.usabilityScoreMax

        # This loop updates the the raw score to actual score using interpolation
        for i in range(len(self.listProperty)):
            self.listProperty[i].priceScore = self.minScore + abs(-((self.listProperty[i].priceScore - self.priceScoreMin) / (self.priceScoreMax - self.priceScoreMin) * self.maxScore - self.maxScore)) * (self.minScore / self.maxScore)
            self.listProperty[i].distanceScore = self.minScore + abs(-((self.listProperty[i].distanceScore - self.distanceScoreMin) / (self.distanceScoreMax - self.distanceScoreMin) * self.maxScore - self.maxScore)) * (self.minScore / self.maxScore)
            self.listProperty[i].usabilityScore = self.minScore + abs((self.listProperty[i].usabilityScore - self.usabilityScoreMin) / (self.usabilityScoreMax - self.usabilityScoreMin) * self.maxScore) * (self.minScore / self.maxScore)
            self.listProperty[i].overallScore = self.minScore + abs(self.listProperty[i].priceScore * weight[0] + self.listProperty[i].distanceScore * weight[1] + self.listProperty[i].usabilityScore * weight[2]) * (self.minScore / self.maxScore)
    ...
    def printScore(self):
        ...
        for i in range(len(self.listProperty)):
            print(str(i+1) + "   " + self.listProperty[self.overallScoreSort[i]].propertyId + "   " + str(round(self.listProperty[self.overallScoreSort[i]].priceScore,1)) + "            " + str(round(self.listProperty[self.overallScoreSort[i]].distanceScore,1)) + "               " + str(round(self.listProperty[self.overallScoreSort[i]].usabilityScore,1)) + "                " + str(round(self.listProperty[self.overallScoreSort[i]].overallScore,1)))
...
class Meta:
    def __init__(self, input):
        self.channel = input['tracking_params']['channel'] if ('tracking_params' in input and 'channel' in input['tracking_params']) else "unknown"
        self.siteSection = input['tracking_params']['siteSection'] if ('tracking_params' in input and 'siteSection' in input['tracking_params']) else "unknown"
        self.city = input['tracking_params']['city'] if ('tracking_params' in input and 'city' in input['tracking_params']) else "unknown"
        self.county = input['tracking_params']['county'] if ('tracking_params' in input and 'county' in input['tracking_params']) else "unknown"
        self.neighborhood = input['tracking_params']['neighborhood'] if ('tracking_params' in input and 'neighborhood' in input['tracking_params']) else "unknown"
        self.searchCityState = input['tracking_params']['searchCityState'] if ('tracking_params' in input and 'searchCityState' in input['tracking_params']) else "unknown"
        self.state = input['tracking_params']['state'] if ('tracking_params' in input and 'state' in input['tracking_params']) else "unknown"
        self.zip = input['tracking_params']['zip'] if ('tracking_params' in input and 'zip' in input['tracking_params']) else "unknown"
        self.srpPropertyStatus = input['tracking_params']['srpPropertyStatus'] if ('tracking_params' in input and 'srpPropertyStatus' in input['tracking_params']) else "unknown"
        self.listingActivity = input['tracking_params']['listingActivity'] if ('tracking_params' in input and 'listingActivity' in input['tracking_params']) else "unknown"
        self.propertyStatus = input['tracking_params']['propertyStatus'] if ('tracking_params' in input and 'propertyStatus' in input['tracking_params']) else "unknown"
        self.propertyType = input['tracking_params']['propertyType'] if ('tracking_params' in input and 'propertyType' in input['tracking_params']) else "unknown"
        self.searchBedrooms = input['tracking_params']['searchBedrooms'] if ('tracking_params' in input and 'searchBedrooms' in input['tracking_params']) else "unknown"
        self.searchMaxPrice = input['tracking_params']['searchMaxPrice'] if ('tracking_params' in input and 'searchMaxPrice' in input['tracking_params']) else "unknown"
        self.searchMinPrice = input['tracking_params']['searchMinPrice'] if ('tracking_params' in input and 'searchMinPrice' in input['tracking_params']) else "unknown"
        self.searchRadius = input['tracking_params']['searchRadius'] if ('tracking_params' in input and 'searchRadius' in input['tracking_params']) else "unknown"
        self.searchHouseSqft = input['tracking_params']['searchHouseSqft'] if ('tracking_params' in input and 'searchHouseSqft' in input['tracking_params']) else "unknown"
        self.searchLotSqft = input['tracking_params']['searchLotSqft'] if ('tracking_params' in input and 'searchLotSqft' in input['tracking_params']) else "unknown"
        self.searchResults = input['tracking_params']['searchResults'] if ('tracking_params' in input and 'searchResults' in input['tracking_params']) else "unknown"
        self.sortResults = input['tracking_params']['sortResults'] if ('tracking_params' in input and 'sortResults' in input['tracking_params']) else "unknown"
        self.searchCoordinates = input['tracking_params']['searchCoordinates'] if ('tracking_params' in input and 'searchCoordinates' in input['tracking_params']) else "unknown"
        self.searchBathrooms = input['tracking_params']['searchBathrooms'] if ('tracking_params' in input and 'searchBathrooms' in input['tracking_params']) else "unknown"
...
class Property:
    ...
    def calculateDistanceScore(self):
        if (isinstance(self.addressLat,float) and isinstance(self.addressLon,float)):
            return math.sqrt(math.pow(self.addressLat - self.targetLat, 2) + math.pow(self.addressLon - self.targetLon, 2)) 
        return 0
    ...
    def __init__(self, input, lat, lon):
        ...
        self.tour = input['virtual_tour']['href'] if ('virtual_tour' in input and 'href' in input['virtual_tour']) else "N/A"
        ...
        self.addressNeighborhood = input['address']['neighborhood_name'] if 'neighborhood_name' in input['address'] else "N/A"
        ...
        self.buildingSize = input['building_size']['size'] if ('building_size' in input and 'size' in input['building_size']) else "N/A"
        self.buildingSizeUnit = input['building_size']['units'] if ('building_size' in input and 'units' in input['building_size']) else "N/A"
        self.lotSize = input['lot_size']['size'] if ('lot_size' in input and 'size' in input['lot_size']) else "N/A"
        self.lotSizeUnit = input['lot_size']['units'] if ('lot_size' in input and 'units' in input['lot_size']) else "N/A"
...
def APIGenerator(self, city, state, ageMin, sqftMax, bedsMin, priceMin, postalCode, radius, priceMax, sqftMin, ageMax):
    ...
    requestString = "/properties/v2/list-for-sale?city=" + city + "&limit=10&offset=0&state_code=" + state + ageMinString + sqftMaxString + bedsMinString + priceMinString + postalCodeString + radiusString + priceMaxString + sqftMinString + ageMax + "&sort=relevance"
...
def main():
    ...
    # Request from api - Sequence: City
    # res = APIGenerator(Philadelphia, 10, 0, PA)
    # input = res.read()
    # data = json.loads(input)       # Pulling Json format from the API 
    ...
    with open('source.txt') as input: # Pulling Json format txts from txt file for testing purposes
        data = json.load(input)       # Remember to comment this line too

    print('*** Starting Point ***')

    propertyList = List(data,40,-73,equalFocused)

    # Test to print out a list of cities (2nd layer test)
    # for i in range(10):
        # print(propertyList.listProperty[i].addressState)
        # print(str(propertyList.listProperty[i].price) + " " + str(propertyList.listProperty[i].buildingSize) + " " + str(propertyList.listProperty[i].lotSize) + " " + str(propertyList.listProperty[i].priceScore) )
        # print(str(propertyList.listProperty[i].addressLat) + " " + str(propertyList.listProperty[i].addressLon) + " " + str(propertyList.listProperty[i].distanceScore))
        # print(str(propertyList.listProperty[i].beds) + " " + str(propertyList.listProperty[i].bathsFull) + " " + str(propertyList.listProperty[i].baths) + " " + str(propertyList.listProperty[i].usabilityScore) )
        # propertyList.listProperty[i].toString()
        
    # propertyList.meta[0].toString()

    propertyList.printScore()

    # for i in range(len(propertyList.overallScoreList)):
        # print(propertyList.overallScoreList[i])

    # print(propertyList.overallScoreSort)
